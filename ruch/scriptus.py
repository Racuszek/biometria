...
# im1=cv.copyMakeBorder(im1, 1, 1, 1, 1, cv.BORDER_CONSTANT, (0, 0, 0))
# cv.imshow('white', im1)
# cv.waitKey()
im2=cv.imread('hand.jpg')
im2=cv.cvtColor(im2, cv.COLOR_BGR2GRAY)
im2=cv.GaussianBlur(im2,(11,11),0)
# im2=cv.copyMakeBorder(im2, 1, 1, 1, 1, cv.BORDER_CONSTANT, (0, 0, 0))
...
# cv.imshow('stencil', hand_stencil)
# cv.waitKey()
...
for i in range(len(contours)):
	...
	if cv.contourArea(contours[i])>1000:
		im2=cv.drawContours(im2, contours, i, (0,0,250),1)
# sheet = np.zeros(img_fin.shape).astype(img_fin.dtype)
# for cnt in contours:
# 	if cv.contourArea(cnt)>1000:
# 		# print(cv.contourArea(cnt))
# 		cv.fillPoly(stencil, [cnt], color)
# 		result=cv.bitwise_and(im1, stencil)
# 		cv.imshow('woo', stencil)
# 		cv.waitKey()
#         # print('saving...')
